...
function prepare_subsoil_data(path_nc, path_csv; bbox::Union{Nothing, NTuple{4, <:Real}} = nothing)
	ds = Dataset(path_nc, "r")
	df = read_params_table(path_csv)
	tables = build_lookup_tables(df)

	data = Dataset(path_nc, "r") do ds
		if bbox !== nothing
			xmin, ymin, xmax, ymax = bbox
			ds = @select(ds, $xmin <= x <= $xmax && $ymin <= y <= $ymax)
		end
		Dict(Symbol(key) => Array(ds[key]) for key in keys(ds))
	end
...
function prepare_reader(path; bbox::Union{Nothing, NTuple{4, <:Real}} = nothing)
	ds = Dataset(path, "r")
...
fetch_field(table, ::Type{PreOverburdenPressure}, domain) = fetch_field(table, :pop, domain)
...
function Base.size(reader::Reader)
	size_x = length(reader.dataset["x"])
	size_y = length(reader.dataset["y"])
	if haskey(reader.dataset, "layer")
